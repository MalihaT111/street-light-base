...
def _get_metrics(cursor, user_id):
    ...
    cursor.execute(
        "SELECT COUNT(DISTINCT created_at::date) FROM reports WHERE user_id = %s AND created_at::date >= %s",
        (user_id, week_start)
    )
    weekly_active_days = cursor.fetchone()[0]
...
def check_and_award_challenges(cursor, user_id):
    ...
    for key, challenge in CHALLENGES.items():
        ...
        if metric_value >= challenge["target"]:
            cursor.execute(
                "INSERT INTO user_challenges (user_id, challenge_key) VALUES (%s, %s)",
                (user_id, sk)
            )
            cursor.execute(
                "INSERT INTO points_log (user_id, report_id, points_earned) VALUES (%s, NULL, %s)",
